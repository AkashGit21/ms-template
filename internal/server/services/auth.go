...
func NewAuthServer(identityStore ReadOnlyIdentityServer, jwtManager *server.JWTManager) authpb.AuthServiceServer {
	return &authServer{identityStore: identityStore, jwtManager: jwtManager}
}
...
func (as *authServer) Login(ctx context.Context, req *authpb.LoginRequest) (*authpb.LoginResponse, error) {
	inputPassword := server.HashPassword(req.GetPassword())
	...
	if user == nil || !reflect.DeepEqual(inputPassword, user.Password) {
		return nil, status.Errorf(codes.NotFound, "incorrect username/password")
	}

	token, err := as.jwtManager.GenerateToken(user)
	if err != nil {
